...
async function clickButton(page, selectionData) {
  const selector =
    typeof selectionData == "object" && selectionData.$
      ? selectionData.$
      : "button";
  ...
  return new Promise<void>((resolve) => {
    const findAndClickButton = async () => {
      ...
      for (const button of buttons) {
        ...
        if (
          buttonText &&
          selectionData.innerText &&
          buttonText.indexOf(selectionData.innerText) == -1
        )
          continue;
        if (selectionData.scrollDown)
      ...
      if (Date.now() - start > 1 * 60 * 1000)
        resolve(); // timeout after 1 minute
...
async function testUnit(page, unit) {
  ...
  for (let k in unitData.output) {
    ...
    if (typeof v === "object") {
      if (v.startsWith) {
        if (!lightningData[k].startsWith(v.startsWith))
          throw `Invalid ${k}. Got ${lightningData[k]} but ${v.startsWith}... was expected`;
      }
    } else {
      if (lightningData[k] != v)
        throw `Invalid ${k}. Got ${lightningData[k]} but ${v} was expected`;
    }
  }
}
  ...
  for (const [unitName, unitData] of Object.entries(Units)) {
    test(unitName, async () => {
      ...
      try {
        ...
        // continue without failing the test if the extension is not found
