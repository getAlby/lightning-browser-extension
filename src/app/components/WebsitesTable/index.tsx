...
type Props = {
  websites: Website[];
  navigateToWebsite: (id: number) => void;
};
...
export default function WebsitesTable({ websites, navigateToWebsite }: Props) {
  const { getFormattedSats, getFormattedNumber } = useSettings();
  const { t: tComponents } = useTranslation("components", {
    keyPrefix: "websites_table",
  });
        ...
        <tbody className="bg-white divide-y divide-gray-200 dark:divide-white/10 dark:bg-surface-02dp">
          {websites.map((website) => (
            <tr
              ...
              onClick={() => navigateToWebsite(website.id)}
            >
                ...
                <div className="flex items-center">
                  <div className="shrink-0">
                    <img
                      ...
                      src={website.imageURL || DEFAULT_IMAGE}
                      alt={website.host}
                      onError={(e) => {
                  ...
                  <div className="ml-4">
                    ...
                    <div className="text-sm text-gray-500 dark:text-neutral-400">
                      {website.host} • {website.paymentsCount}{" "}
                      {tComponents("payments")}{" "}
                      {website.paymentsAmount > 0 && (
                        <span>{getFormattedSats(website.paymentsAmount)}</span>
                      )}
                    </div>
                  </div>
                </div>
              ...
              <td className="px-6 py-6 whitespace-nowrap text-right">
                {website.totalBudget > 0 && (
                  <div className="ml-40">
                    <p className="text-lg text-gray-500 mb-0 dark:text-neutral-400">
                      {getFormattedNumber(website.usedBudget)} /{" "}
                      {getFormattedNumber(website.totalBudget)}{" "}
                      {tCommon("sats", { count: website.usedBudget })}{" "}
                      {tComponents("used")}
                    </p>
                    <div className="relative mt-2 ml-auto">
                      <div className="flex items-center justify-end">
                        <div className="w-40">
                          <Progressbar percentage={website.percentage} />
                        </div>
                        <span
