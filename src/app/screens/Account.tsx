...
import api, { AccountByIdRes } from "~/common/lib/api";
...
function ManageAccount() {
  ...
  const [account, setAccount] = useState<AccountByIdRes>();
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      if (id) {
        const response = await msg.request<AccountByIdRes>("getAccountById", {
          id,
        });
        setAccount(response);
        setAccountName(response.name);
        setNostrPrivateKey(response.nostrPrivateKey as string);
        setLoading(false);
      }
    } catch (e) {
      console.error(e);
