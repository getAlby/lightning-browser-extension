...
const channelUrlMatcher =
  /^https:\/\/www\.youtube.com\/(((channel|c)\/)|@)([^/]+).*/;
const videoUrlMatcher = /^https:\/\/www\.youtube.com\/watch.*/;
...
const battery = async (): Promise<void> => {
  ...
  // Therefore the callback needs to also be called manually.
  youtubeDOMChanged();
};
...
async function youtubeDOMChanged() {
  let info = (await getVideoInfo()) || (await getChannelInfo());
  if (!info?.lnurl) {
...
async function getVideoInfo(): Promise<PublisherInfo | null> {
  if (!document.location.toString().match(videoUrlMatcher)) {
    return null;
  }
  ...
  // we check for the @-sign to try to limit the possibility to match some invalid text (e.g. random emoji usage)
    ...
    const match = channelLink.href.match(channelUrlMatcher);
    if (match) {
...
async function getChannelInfo(): Promise<PublisherInfo | null> {
  ...
  if (headerLink) {
    ...
    lnurl = await findLnurlFromYouTubeAboutPage(match[1], match[4]);
  }
